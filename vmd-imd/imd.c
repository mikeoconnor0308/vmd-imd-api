...
char * imd_deserialize_energies(char* buffer, IMDEnergies* energies)
{
  buffer = imd_deserialize_int(buffer, &energies->tstep);
  buffer = imd_deserialize_float(buffer, &(energies->T));
  buffer = imd_deserialize_float(buffer, &energies->Etot);
  buffer = imd_deserialize_float(buffer, &energies->Epot);
  buffer = imd_deserialize_float(buffer, &energies->Evdw);
  buffer = imd_deserialize_float(buffer, &energies->Eelec);
  buffer = imd_deserialize_float(buffer, &energies->Ebond);
  buffer = imd_deserialize_float(buffer, &energies->Eangle);
  buffer = imd_deserialize_float(buffer, &energies->Edihe);
  buffer = imd_deserialize_float(buffer, &energies->Eimpr);
  return buffer;
}

char * imd_serialize_energies(char* buffer, const IMDEnergies* energies)
{
  buffer = imd_serialize_int(buffer, energies->tstep);
  buffer = imd_serialize_float(buffer, energies->T);
  buffer = imd_serialize_float(buffer, energies->Etot);
  buffer = imd_serialize_float(buffer, energies->Epot);
  buffer = imd_serialize_float(buffer, energies->Evdw);
  buffer = imd_serialize_float(buffer, energies->Eelec);
  buffer = imd_serialize_float(buffer, energies->Ebond);
  buffer = imd_serialize_float(buffer, energies->Eangle);
  buffer = imd_serialize_float(buffer, energies->Edihe);
  buffer = imd_serialize_float(buffer, energies->Eimpr);
  return buffer;
}
...
int imd_send_energies(void *s, const IMDEnergies *energies) {
  ...
  char *buf = (char *)malloc(sizeof(char) * size);
  */
  int32 size = HEADERSIZE + IMDENERGIESSIZE;
  char *buf = (char *)malloc(sizeof(char) * size);
  fill_header((IMDheader *)buf, IMD_ENERGIES, 1);
  char* ptr = imd_serialize_energies(buf + HEADERSIZE, energies);
...
int imd_recv_energies(void *s, IMDEnergies *energies) {
  int32 size = IMDENERGIESSIZE;
  char *buf = (char *)malloc(sizeof(char) * size);
  int error = (imd_readn(s, buf, IMDENERGIESSIZE)) != IMDENERGIESSIZE;
  if (error == 1) return 1;
  imd_deserialize_energies(buf, energies);
  free(buf);
  return 0;
}
